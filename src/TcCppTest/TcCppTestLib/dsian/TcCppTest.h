...
namespace dsian
{
    ...
    public:
        ...
        static std::function<void(char const*)> TestFixture::Print;
        static void Printf(char const* string, ...);

        static bool ExecuteAllTests(char const* groupFilter = nullptr, char const* nameFilter = nullptr, OutputMode output = OutputMode::Normal, std::function<void(const char*)> printCallback = nullptr);
        static bool ExecuteAllTests(OutputMode output) { return ExecuteAllTests(nullptr, nullptr, output, nullptr); }
        static bool ExecuteAllTests(std::function<void(const char*)> printCallback) { return ExecuteAllTests(nullptr, nullptr, OutputMode::Normal, printCallback); }
        static bool ExecuteAllTests(OutputMode output, std::function<void(const char*)> printCallback) { return ExecuteAllTests(nullptr, nullptr, output, printCallback); }

        static bool ExecuteTestGroup(char const* groupFilter, OutputMode output = OutputMode::Normal, std::function<void(const char*)> printCallback = nullptr) { return ExecuteAllTests(groupFilter, nullptr, output, nullptr); }
...
struct TOK(group, name) final : public dsian::fixture { \
...
#define TOK(a, b) TOK2(a, b)
    ...
    inline std::string PrefixMessage(const char* file, const char* line, const char* message)
    {
        std::string prefix = strrchr(file, '\\') ? strrchr(file, '\\') + 1 : file;
        prefix.append("(");
        prefix.append(line);
        prefix.append("): Condition [%s] Failed. ");
    ...
    //---------------------------------------------------------------------------------
#define TEST_TYPE_TO_STRING(var, arg) *TypeToStringFallback(dsian::TypeToString(var), STR(arg))
#define TEST_ERROR_PREFIX_(message) dsian::PrefixMessage(__FILE__, STR(__LINE__), message).c_str()
#define TEST_ERROR_(message, ...) do { dsian::TestFixture* __fx = dsian::TestFixture::GetCurrentTest(); __fx->AddError(); __fx->LogMessage(TEST_ERROR_PREFIX_(message), ##__VA_ARGS__); ERROR_ACTION; } while(0)
#define TEST_BEGIN_(a) do { auto const& test_value_ = a
#define TEST_CHECK_(cond, condtext, message, ...) do { dsian::TestFixture::GetCurrentTest()->AddTest(); if (!(cond)) TEST_ERROR_(message, condtext, ##__VA_ARGS__); } while(0)
...
#define TEST_STR_EQ(a, b) do { if(!dsian::TestFixture::GetCurrentTest()->TestStrings(a, b, TEST_ERROR_PREFIX_("\n%s\n%s\n%s"), STR(a) " == " STR(b))) { ERROR_ACTION; } } while(0)
#define TEST_CLOSE(a, b, eps) TEST_BEGIN_(dsian::TestDifference(a, b)); TEST_CHECK_(test_value_ <= eps, STR(a) " Close to " STR(b), "Difference of %s is greater than expected amount of " STR(eps) ".", TEST_TYPE_TO_STRING(test_value_, dsian::TestDifference(a, b))); TEST_END_
#define TEST_DIFFERS(a, b, eps) TEST_BEGIN_(dsian::TestDifference(a, b)); TEST_CHECK_(test_value_ >= eps, STR(a) " Differs from " STR(b), "Difference of %s is less than expected amount of " STR(eps) ".", TEST_TYPE_TO_STRING(test_value_, dsian::TestDifference(a, b))); TEST_END_
